...
def get_names(p, args, date, record_tensorboard, save_model):
    ...
    inf_lamda = f'_lamda{p.agent.lamda}' if p.agent.name == 'GBDAC' else ''
    ...
    if record_tensorboard:
        name_tb = f'data_NeurIPS/{p.agent.name}/{p.env.name}/'\
    ...
    if save_model:
        if p.agent.name == 'GBDAC':
            save_directory = f'data_NeurIPS/{p.agent.name}/'\
        ...
        name_save = None
    if p.agent.name == 'GBDAC':
        name_logger_folder = (f'data_NeurIPS/{p.agent.name}/'
...
def get_names_eval(p):
    ...
    if p.agent.name == 'GBDAC':
        name_logger_folder = (f'data_NeurIPS_tables/'
                              f'{p.agent.name}_{inf_distortion}/'
                              f'{p.env.name}/eval')
    else:
        name_logger_folder = (f'data_NeurIPS_tables/{p.agent.name}/'
                              f'{p.env.name}/eval')
