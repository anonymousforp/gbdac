...
import numpy as np
import torch
from diffusion import Diffusion, Diffusion_BC
from helpers import MLP
from oraaclib.agent import GBDAC
from oraaclib.environment import get_env
from oraaclib.util import get_dict_hyperparams, gbdac_rollout
...
if not args.eval:
    with open('json_params/GBDAC/'+config_name) as f:
        params = json.load(f)
...
if args.eval:
    ...
    if args.record:
        ...
        env = gym.wrappers.Monitor(
            env, directory=f'recording/{p.agent.name}/'
...
if p.agent.name == 'RAAC':
    ...
    policy = ORAAC_Actor(dim_state=dim_state, dim_action=dim_action,
                         ...
                         lamda=p.agent.lamda, tau=p.agent.TARGET_UPDATE_TAU)
# else:
#     policy = Diffusion(state_dim=dim_state, action_dim=dim_action, model=MLP(state_dim=dim_state, action_dim=dim_action, device=device),
...
if args.eval:
    ...
    vae_diffusion.load_state_dict(model_net['vae'])
